...
echo ""
	...
	echo "ESTE SCRIPT IRÁ REALIZAR AS SEGUINTES CONFIGURAÇÕES:"
...
# VERIFICAR ESPAÇO DISPONÍVEL NO VOLUMEGROUP LVM
...
##   	  vgdisplay  | grep Free | awk -F "<" {'print $2'}
	...
	VGVIEW=$(vgdisplay  | grep Free | cut -d "<" -f2)
	VG=$(vgdisplay  | grep Free | cut -d "<" -f2 | cut -d "," -f1)
	echo "ESPAÇO DISPONÍVEL NO VOLUMEGROUP - $DATA: $VGVIEW "
echo ""
	if [ "$VG" -ge "$LIMIT" ]; then
	  echo " +++ O VOLUMEGROUP POSSUI ESPAÇO DISPONÍVEL PARA CRIAR O LVM +++ "
echo ""
	 CRIALVM
	else
	  echo " --- ESPAÇO NO VOLUME GROUP INDISPONÍVEL --- "
echo ""
	 EXITVOLUME
	fi
...
EXITVOLUME() {
	echo "CONTACTE O RESPONSÁVEL PARA EXPANDIR OU CONFIGURAR NOVO VOLUME GROUP - LVM"
	exit
}
