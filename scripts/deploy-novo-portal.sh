...
# PRÉ-REQUISITO: Configuração de DNS "homolog-nome.mma.gov.br"
#------------------------------------------------------------------#
...
echo ""
	echo "VALIDA URL:"
	CURL=$(curl -s -o /dev/null -w "%{http_code}" http://homolog-$NOMEVOL.mma.gov.br)
	if [ $CURL == 200 ]; then
	  echo "SISTEMA DISPONÍVEL - STATUS: $CURL"
echo""
	  echo "----------------------------------------------"
	  echo " ACESSE: http://homolog-$NOMEVOL.mma.gov.br "
	  echo "----------------------------------------------"
echo ""
	elif [ $CURL == 302 ]; then
...
echo ""
	  ...
	  echo "TENTE ACESSAR A URL: http://homolog-$NOMEVOL.mma.gov.br"
	  echo "-----------------------------------------------------------"
echo ""
echo ""
	fi
